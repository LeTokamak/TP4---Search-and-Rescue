...
class Climber_perso_v1(Climber):
    def __init__(self, x, y, model, environment):
        super().__init__(x, y, model, environment)
...
class Climber_gauche_v1(Climber_perso_v1):
    def __init__(self, x, y, model, environment):
        super().__init__(x, y, model, environment)
...
class Climber_droite_v1(Climber_perso_v1):
    def __init__(self, x, y, model, environment):
        super().__init__(x, y, model, environment)
...
class Speeder_chef_v2(Speeder):
    ...
    def mission_RECUPERER_OBJET(self):
        self.goto(self.objet_objectif.x, self.objet_objectif.y)
        
        if [self.x, self.y] == [self.objet_objectif.x, self.objet_objectif.y]:
            self.take(self.objet_objectif)
            self.objet_porte = self.objet_objectif
            self.objet_objectif = None
            self.mission        = None
            self.etape          = 2
    ...
    def step(self):
        ...
        # Etape 2 : Le Speeder attend un ordre de mission
            ...
            objets_non_recuperes = [objet 
                                        for objet in self.objets_connus 
                                        if  objet not in self.objets_sur_personne]
            
            # Si le Speeder sait où est la personne et qu'il porte un objet
            if self.personne != None and self.objet_porte != None:
                self.mission = OBJET_TO_PERSONNE
                self.objet_objectif = self.objet_porte
                self.etape = 3
                self.etape_mission = 0
            ...
            # Si le Speeder ne sais pas où est la personne
            elif self.objet_porte == None and len(objets_non_recuperes) != 0 :
                    
                self.mission = RECUPERER_OBJET
                self.objet_objectif = objets_non_recuperes[0]
                self.etape          = 3
                    
                self.mission_RECUPERER_OBJET()

        
        # Etape 3 : Le Speeder est en mission
        elif self.etape == 3:
            if self.mission == RECUPERER_OBJET:
                self.mission_RECUPERER_OBJET()
...
class Climber_perso_v2(Climber):
    def __init__(self, x, y, model, environment):
        super().__init__(x, y, model, environment)
        # Variable de communication
        self.recoit_message = False
    ...
    def deplacement_recherche(self):
        ...
        if   (not self.recherche_terminee or  self.etape <= -1) and (self.x != destination_x or self.y != destination_y) :
            self.goto(destination_x, destination_y)
        elif (not self.recherche_terminee and self.etape >=  0) :
            self.etape += 1
            if self.etape == len(self.trajectoire):
                self.etape = -1
        elif (not self.recherche_terminee and self.etape <= -1) :
            self.etape -= 1
            if self.etape == -len(self.trajectoire)-1:
                self.etape = 0
                self.recherche_terminee = True
...
class Climber_gauche_v2(Climber_perso_v2):
    def __init__(self, x, y, model, environment):
        super().__init__(x, y, model, environment)
        self.trajectoire = liste_etape_climber_gauche
        self.coordonnees_position_rapport = [3.25*(taille_case)+centre_case, 
...
class Climber_droite_v2(Climber_perso_v2):
    def __init__(self, x, y, model, environment):
        super().__init__(x, y, model, environment)
        self.trajectoire = liste_etape_climber_droite
        self.coordonnees_position_rapport = [3.75*(taille_case)+centre_case, 
